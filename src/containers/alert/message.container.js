...
import { DeleteError, DeleteSuccess } from '../../actions/shared.action';
    ...
    componentDidMount() {
        setTimeout(() => {
            this.deleteErrorMessage(
                this.props.error
                    ? this.props.error.id
                    : this.props.success ? this.props.success.id : ''
            );
    ...
    render() {
        return (
            <div
                ...
                style={{ background: this.props.error ? '#c12222' : '#98962c' }}
            >
                <div className="error_name_wrapper">
                    <span className="error_name">
                        {this.props.error
                            ? this.props.error.message.toString()
                            : this.props.success
                                ? this.props.success.message &&
                                this.props.success.message.toString()
                                : ''}
                    </span>
                </div>
                <div className="err_delet_btn_wrappper">
                    <span
                        className="err_delet_btn"
                        onClick={
                            this.props.error
